...
//		}
		...
		// System.out.println("timer interrupt detected. 0xFF0F =
		// "+Utils.toHex2(mem.RAM[0xFF0F]));
		// }

		// System.out.println("0xFF00 (input) "+mem.peek(0xff00));
		// System.out.println("0xFF44 (scanline) "+mem.peek(0xFF44));

		// System.out.println("LCD Stat " +
		// Utils.toHex2(mem.RAM[CPU.ADDR_FF41_LCD_STAT]));
...
int carryIn = 0;
		...
		case SBC:
			...
			if ((wrk&0xFF)>0) unsetFlag(FLAG_ZERO); else setFlag(FLAG_ZERO);        
			if ((wrk&0x100)>0) setFlag(FLAG_CARRY); else unsetFlag(FLAG_CARRY);        
			if (((A^ac1.val^wrk)&0x10)!=0) setFlag(FLAG_HALFCARRY); else unsetFlag(FLAG_HALFCARRY);
